...
class Agent():
    ...
    def step(self, state, action, reward, next_state, done, timestep):
        ...
        for agnt in range(self.num_agents):
            self.memory.add(state[agnt,:], action[agnt,:], reward[agnt], next_state[agnt,:], done[agnt])
            #self.memory.add(state, action, reward, next_state, done)

        # Learn, if enough samples are available in memory
        if len(self.memory) > BATCH_SIZE:
            if timestep % 20 ==0: # update every 20 timesteps
...
class OUNoise:
    ...
    def __init__(self, size, seed, mu=0., theta=0.15, sigma=0.09):
